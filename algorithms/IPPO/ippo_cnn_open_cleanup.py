...
def make_train(config):
    # IPPO is the memoryless control for RPPO on this environment, so it has
    # to train on exactly the same half of the rule space — otherwise the
    # comparison prices a different training distribution, not the absence of
    # memory. The split is deterministic given RULE_SPLIT_SEED, so the two
    # runs really do see the same rules.
    ...
    env = opensocialjax.make(config["ENV_NAME"], rule_pool=train_rules,
                             **config["ENV_KWARGS"])
...
SINGLE_RUN_KWARGS = {"wandb_name": "ippo_cnn_open_cleanup"}
